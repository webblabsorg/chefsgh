...
router.patch('/:id', async (req, res) => {
  try {
    ...
    params.push(id);
    await pool.execute(`UPDATE registrations SET ${updates.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, params);
    const [rows] = await pool.query(
      `SELECT r.id, r.membership_id, r.membership_status, r.membership_expiry FROM registrations r WHERE r.id = ?`,
      [id]
    );
