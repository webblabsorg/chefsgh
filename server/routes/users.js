...
router.get('/:id', async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      `SELECT u.* FROM users u WHERE u.id = $1 LIMIT 1`,
      [id]
    );
...
router.post('/', async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(`SELECT * FROM users WHERE id = $1`, [result.insertId || u.id]);
...
router.patch('/:id', async (req, res) => {
  try {
    ...
    params.push(id);
    await pool.execute(`UPDATE users SET ${updates.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = $1`, params);
